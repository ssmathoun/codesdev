...
expandedIds, handleExpandedIds, itemLookup} : { data: folderStructureData[], getPath: (id: number) => number[], projectName: string, handleOpenTab: (id: number) => void, openedId: number | null, handleOpenedId: (id: number) => void, openedFileTabsId: number[], handleOpenedFileTabsId: (id: number, toggle?: boolean) => void , expandedIds: number[], handleExpandedIds: (id: number) => void, itemLookup: Map<number, folderStructureData>}) {
    ...
    return (
        <>
            <div className="flex h-9 shrink-0 bg-ide-bg text-white items-center text-[14px] overflow-x-auto overflow-y-hidden flex-nowrap custom-scrollbar">
            {/* Opened File Tabs */}
            ...
            {/* Breadcrumbs */}
            <div ref={breadcrumbRef} className="flex h-4 shrink-0 bg-[#1E1E1E] text-zinc-400 items-center text-[14px] px-3 py-3 whitespace-nowrap overflow-x-auto overflow-y-hidden flex-nowrap custom-scrollbar select-none">
