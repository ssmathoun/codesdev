...
export default function HomePage() {
  ...
  const [projects, setProjects] = useState([
    { id: "p1", name: "project-1", date: "Jan 14, 2026", category: "mine", owner: mockUsers["You"], 
      sharedWith: [mockUsers["Alex"], mockUsers["Jordan"], mockUsers["Sarah"], mockUsers["Mike"]] },
    { id: "p2", name: "project-2", date: "Dec 30, 2025", category: "mine", owner: mockUsers["You"], sharedWith: [] },
    { id: "p3", name: "project-3", date: "Dec 16, 2025", category: "shared-in", owner: mockUsers["ProfSmith"], 
      sharedWith: [mockUsers["You"], mockUsers["Alex"]] },
    { id: "p4", name: "project-4", date: "Dec 07, 2025", category: "mine", owner: mockUsers["You"], sharedWith: [] },
    { id: "p5", name: "project-5", date: "Nov 22, 2025", category: "shared-out", owner: mockUsers["You"], 
  ...
  const openProject = (id: string, name: string) => {
    navigate(`/editor/${id}`, { state: { projectName: name } });
  };
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    setProjects([{ id, name, date: "Jan 15, 2026", category: "mine", owner: mockUsers["You"], sharedWith: [] }, ...projects]);
    setIsAddModalOpen(false); setNewItemName("");
    openProject(id, name);
  };
          ...
          <div className="max-w-300 mx-auto">
            ...
            {viewMode === "list" ? (
              <div className="w-full border border-white/6 rounded-lg overflow-hidden bg-[#0a0a0a] shadow-sm">
                ...
                <div className="divide-y divide-white/4">
                  {filteredProjects.map(p => (
                    <ProjectListRow key={p.id} project={p} onOpen={() => openProject(p.id, p.name)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                {filteredProjects.map(p => (
                  <ProjectCard key={p.id} project={p} onOpen={() => openProject(p.id, p.name)}
                    onRename={() => { setSelectedProjectId(p.id); setNewItemName(p.name); setIsRenameModalOpen(true); }}
                    onDelete={() => { setSelectedProjectId(p.id); setIsDeleteModalOpen(true); }}
                    onManageSharing={() => { setSelectedProjectId(p.id); setIsShareModalOpen(true); }}
                  />
...
function Avatar({ collaborator, size = "md" }: { collaborator?: Collaborator, size?: "sm" | "md" }) {
    ...
    return <div className={`${sizeClasses} rounded-full bg-zinc-800 border border-black/50 flex items-center justify-center font-bold text-zinc-400`}>{collaborator.name[0]}</div>;
}
...
function AvatarStack({ collaborators, onClick }: { collaborators: Collaborator[], onClick: (e: React.MouseEvent) => void }) {
    ...
    return (
        <div className="flex -space-x-1.5 hover:opacity-80 transition-opacity cursor-pointer shrink-0" onClick={onClick}>
            ...
            {remaining > 0 && <div className="w-5 h-5 rounded-full bg-zinc-700 border border-black/50 flex items-center justify-center text-[9px] font-bold text-zinc-300 ring-2 ring-[#0a0a0a] relative z-0">+{remaining}</div>}
        </div>
    )
}
      ...
      <div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-white/6">
           <div className="flex flex-col gap-1.5 min-w-0">
             ...
             {(isSharedIn || isSharedOut) && <AvatarStack collaborators={project.sharedWith} onClick={(e) => { e.stopPropagation(); onManageSharing(); }} />}
